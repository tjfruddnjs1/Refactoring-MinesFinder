...
public GW_2playerMode(Minefield minefield) {
	...
	user1.setDocument(new JTextFieldLimit(5));
	user2.setDocument(new JTextFieldLimit(5));
	...
	if(player1Turn > player2Turn) {
      ...
      KeyListener keyListener = new KeyListener() {
          ...
          public void keyPressed(KeyEvent e) {
              ...
              int x = botao.getCol();
              int y = botao.getLine();
              if (e.getKeyCode() == KeyEvent.VK_W && x > 0 || e.getKeyCode() == KeyEvent.VK_UP && x>0) {
                  buttons[x - 1][y].requestFocus();
              } else if (e.getKeyCode() == KeyEvent.VK_A && y > 0 || e.getKeyCode() == KeyEvent.VK_LEFT && y>0) {
                  buttons[x ][y - 1].requestFocus();
              } else if (e.getKeyCode() == KeyEvent.VK_S && x < minefield.getWidth() - 1 || 
            		  e.getKeyCode() == KeyEvent.VK_DOWN && x < minefield.getWidth() -1) {
            	  buttons[x + 1][y].requestFocus();
              } else if (e.getKeyCode() == KeyEvent.VK_D && y < minefield.getHeight() - 1 || 
            		  e.getKeyCode() == KeyEvent.VK_RIGHT && y < minefield.getHeight() - 1) {
                  buttons[x][y + 1].requestFocus();
              }
                else if(e.getKeyCode() == KeyEvent.VK_E || e.getKeyCode() == KeyEvent.VK_M) {
                	if (minefield.getGridState(x, y) == minefield.COVERED) {
